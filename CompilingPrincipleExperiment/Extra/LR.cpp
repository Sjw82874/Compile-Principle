...
void print_ans_table_on_consle();							//控制台打印 
...
void print_ans_table(){
	...
	int wid=8;
	outfile.width(wid*T.size()+wid);
	outfile<<"action";outfile<<"|";
	outfile.width(wid*V.size()-wid);
	outfile<<"goto"<<endl;
	for(int i=0;i<T.size();i++) outfile.width(wid),outfile<<T[i];
	outfile.width(wid);outfile<<"#";outfile<<"|";
	for(int i=1;i<V.size();i++) outfile.width(wid),outfile<<V[i];
	outfile<<endl;
	for(int i=0;i<C.size();i++){
		for(int j=V.size();j<=token.size();j++){
			outfile.width(wid);
			//999特指分析结束 
			if(action[i][j]==999) outfile<<"end";
			else outfile<<action[i][j];
		} outfile<<"|";
		for(int j=1;j<V.size();j++) outfile.width(wid),outfile<<go_to[i][j];
		outfile<<endl;
	} 
	outfile.close();
}

void print_ans_table_on_consle(){
	int wid=6;
...
//	if(tstr.size()!=1) return false;
	...
	if(tt=="FLOAT10") return true;
	if(tt=="FLOAT16") return true; 
	if(tt=="FLOAT8") return true;
	return false;
}
...
//		cout<<s_top<<" "<<itoken[token_a]<<" "<<next_step<<endl;
		...
		if(next_step>0&&next_step!=999){
			...
			//输出产生式 
			outfile<<itoken[G.lstr[next_step]]<<"-> ";
			for(int i=0;i<tstr.size();i++){
				outfile<<itoken[tstr[i]]<<" ";
			}
			//如果有终结符需要输出，就弹栈 
			if(need_output(itoken[tstr[0]])) outfile<<strsta.top(),strsta.pop();
			outfile<<endl;
		}
		//结束 
